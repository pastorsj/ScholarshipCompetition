...
contract.df %>%
  group_by(prime_awardee_name) %>%
  count(name = 'unique_contractors')
# Number of unique sub contractors
contract.df %>%
  group_by(subawardee_name) %>%
  count(name = 'unique_subcontractors')
...
# Timeline
cost.contracts.2.df <- contract.df %>%
  select(subaward_action_date, subaward_amount) %>%
  group_by(subaward_action_date) %>%
  summarize(contracts_sold = n(),
            total_cost_day = sum(subaward_amount)) %>%
  arrange(subaward_action_date) %>%
  mutate(total_cost = cumsum(total_cost_day))

write.csv(cost.contracts.2.df %>% mutate(total_cost = as.character(total_cost)), "./visualization_data/contract_costs_2_time.csv", row.names = FALSE)

# Contracts
write.csv(contract.df %>% select(subaward_action_date, subawardee_name, subaward_description, subaward_amount), "./visualization_data/contract_info.csv", row.names = FALSE)
```
...
# Filtered out Alliant
```{r}
sunburst.df <- data.frame(parent = c(""), id = c("All"), name = 'DOD', value = c(0), final_layer = FALSE)

first.layer <- network.awards.df %>%
  filter(subawardee_name != 'ALLIANT TECHSYSTEMS OPERATIONS LLC') %>%
  group_by(prime_award_awarding_sub_agency_name) %>%
  summarize(value = sum(subaward_amount)) %>%
  mutate(
    parent = "All",
...
write.csv(sunburst.viz.df %>% mutate(value = as.character(value)), "./visualization_data/award_filtered_relationships.csv", row.names = FALSE)
```
...
words.group.df <- description.filtered.contracts.df %>% 
  ...
  count(name = 'value') %>%
  ungroup() %>%
  complete(number_of_words = 1:7, fill = list(value = 0))
...
## Determining the unique subcontractors
```{r}
important.subcontractors.df <- description.filtered.contracts.df %>%
  select(subawardee_name, number_of_words, extracted_words, subaward_description) %>%
  group_by(extracted_words, subawardee_name) %>%
  count(name = 'Unique Words') %>%
  ungroup() %>%
  separate_rows(extracted_words) %>%
  group_by(extracted_words) %>%
  filter(n() == 1) %>%
  group_by(subawardee_name) %>%
  count(name = 'temp') %>%
  ungroup() %>%
...
important.filtered.contracts.df <- description.filtered.contracts.df %>%
  filter(subawardee_name %in% important.subcontractors.df$subawardee_name)
```


### Recreate subcontract network (Important)
```{r}
top.description.based.subcontracts.df <- important.filtered.contracts.df %>%
  group_by(subawardee_name) %>%
  summarize(NumberOfContracts = n(),
            TotalCostOfAllContracts = sum(subaward_amount)) %>%
  arrange(desc(TotalCostOfAllContracts))

second.description.based.layer <- important.filtered.contracts.df %>%
  select(subcontract_id, subawardee_name) %>%
  filter(subawardee_name %in% top.description.based.subcontracts.df$subawardee_name) %>%
  mutate(from = as.character(subcontract_id),
         to = as.character(subawardee_name)) %>%
...
first.description.based.layer <- important.filtered.contracts.df %>% 
  select(subcontract_id, prime_awardee_name) %>%
  filter(subcontract_id %in% second.description.based.layer$from) %>%
  mutate(from = as.character(prime_awardee_name),
         to = as.character(subcontract_id)) %>%
...
write.csv(do.call('rbind', list(first.description.based.layer, second.description.based.layer)), "./visualization_data/important_subcontract_description_based_network.csv", row.names = FALSE)
```

### Recreate Description Based Cost Nodes (Important)
```{r}
all.filtered.description.based.costs.df <- important.filtered.contracts.df %>%
  select(subcontract_id, subaward_description, extracted_words, subaward_amount) %>%
  rename(id = subcontract_id,
         description = subaward_description,
         cost = subaward_amount) %>%
...
all.filtered.description.based.subcontracters.df <- important.filtered.contracts.df %>%
  select(subawardee_name, subaward_amount) %>%
  group_by(subawardee_name) %>%
  summarize(cost = sum(subaward_amount)) %>%
  ungroup() %>%
...
result.filtered.description.costs.df <- result.filtered.description.costs.df %>%
  mutate(cost = as.character(cost))
write.csv(result.filtered.description.costs.df, "./visualization_data/important_all_filtered_description_based_costs.csv", row.names = FALSE)
```