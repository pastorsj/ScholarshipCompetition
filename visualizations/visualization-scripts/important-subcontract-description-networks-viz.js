(async () => {
  ...
  const costData = await fetch(
    `${url}/visualization_data/important_all_filtered_description_based_costs.csv`
  )
    .then((result) => result.text())
    .then((text) => Papa.parse(text, { header: true, skipEmptyLines: true }))
    .then((data) => data.data);
  ...
  Highcharts.chart("important-description-networks-container", {
    chart: {
      type: "networkgraph",
      height: "800px",
    },
    title: {
      text: "Subcontractors that build unique products for their subcontract based on the description",
    },
    subtitle: {
